package com.kryxhub.kryxhub.communication.service;
...
public class EmailService {
    ...
    public void sendPasswordResetOtp(String toEmail, String otpCode) {
        ...
        String htmlBody = "<div style='font-family: Arial, sans-serif; text-align: center;'>"
                + "<h2>KryxHub Password Reset</h2>"
                + "<p>We received a request to reset your password. Here is your authorization code:</p>"
                + "<h1 style='color: #E53935; letter-spacing: 5px;'>" + otpCode + "</h1>"
                + "<p>This code will expire in <strong>10 minutes</strong>.</p>"
                + "<p style='font-size: 12px; color: gray;'>If you did not request a password reset, please ignore this email or contact support.</p>"
                + "</div>";

        CreateEmailOptions params = CreateEmailOptions.builder()
                .from("onboarding@resend.dev")
                .to(toEmail)
                .subject("KryxHub: Your Password Reset Code")
                .html(htmlBody)
                .build();

        try {
            CreateEmailResponse data = resend.emails().send(params);
            System.out.println("Password Reset Email sent! Resend ID: " + data.getId());
        } catch (ResendException e) {
            System.err.println("CRITICAL: Failed to send reset email via Resend: " + e.getMessage());
            throw new RuntimeException("Failed to send the reset email. Please try again.");
        }
    }
}